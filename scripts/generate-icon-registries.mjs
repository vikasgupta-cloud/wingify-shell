...
const PHOSPHOR = {
  ...
  UserRound: "User",
  // Fallbacks that previously resolved to Question — map to real Phosphor glyphs
  Activity: "Pulse",
...
/** Per-icon SSR imports — avoids bundling the 5MB @phosphor-icons/react barrel. */
function writePhosphorRegistryFile(map) {
  const uniqueIcons = [...new Set(Object.values(map))].sort();
  const imports = uniqueIcons
    .map(
      (iconName) =>
...
`;
  fs.writeFileSync(path.join(outDir, "phosphorRegistry.ts"), content);
}
...
async function main() {
  ...
  writePhosphorRegistryFile(phosphorMap);
