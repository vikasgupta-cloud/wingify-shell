...
import { PERSONALIZE_COLUMNS, type PersonalizeColumnId } from "../../config/personalizeColumns";
...
const isLocked = (id: PersonalizeColumnId) =>
  PERSONALIZE_COLUMNS.find((c) => c.id === id)?.locked === true;

export default function PersonalizeColumnConfig() {
  const { visibleColumns } = useActivePersonalizeViewState();
  const updateDraft = usePersonalizeViewsStore((s) => s.updateActiveViewDraft);
  const resetColumns = usePersonalizeViewsStore((s) => s.resetActiveViewColumns);
  const rowDensity = usePersonalizeTableStore((s) => s.rowDensity);
  const setRowDensity = usePersonalizeTableStore((s) => s.setRowDensity);
  const [dragIndex, setDragIndex] = useState<number | null>(null);
  const [dropIndex, setDropIndex] = useState<number | null>(null);

  const byId = new Map(PERSONALIZE_COLUMNS.map((c) => [c.id, c]));
  const available = PERSONALIZE_COLUMNS.filter((c) => !visibleColumns.includes(c.id));

  const toggleColumn = (id: PersonalizeColumnId) => {
