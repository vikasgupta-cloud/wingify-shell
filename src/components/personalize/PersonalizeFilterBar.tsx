...
import {
  getPersonalizeFilterFields,
  type PersonalizeFilter,
  type PersonalizeFilterField,
  type PersonalizeFilterOp,
} from "../../config/personalizeFilters";
import { PERSONALIZE_GROUP_FIELDS, type PersonalizeGroupField } from "../../config/personalizeGrouping";
...
const OP_LABELS: Record<Exclude<PersonalizeFilterOp, "is">, string> = {
  isAnyOf: "is any of",
  isNoneOf: "is none of",
};

function opLabel(op: PersonalizeFilterOp): string {
  return op === "is" ? "is" : OP_LABELS[op];
}

export default function PersonalizeFilterBar() {
  const rows = useVisiblePersonalizations();
  const fields = getPersonalizeFilterFields(rows);
  const { filters, groupBy } = useActivePersonalizeViewState();
  const updateDraft = usePersonalizeViewsStore((s) => s.updateActiveViewDraft);

  const fieldDef = (field: PersonalizeFilterField) => fields.find((f) => f.field === field);
  const fieldLabel = (field: PersonalizeFilterField) => fieldDef(field)?.label ?? field;

  const addFilter = (field: PersonalizeFilterField) => {
    updateDraft({
      filters: [...filters, { field, op: "isAnyOf", value: [] }],
    });
  };

  const setFilterAt = (index: number, patch: Partial<PersonalizeFilter>) => {
    updateDraft({
  ...
  const groupLabel = PERSONALIZE_GROUP_FIELDS.find((g) => g.id === groupBy)?.label;
...
function filterValueLength(filter: PersonalizeFilter): number {
...
function GroupMenu({
  ...
  value: PersonalizeGroupField | null;
  onSelect: (g: PersonalizeGroupField | null) => void;
}) {
  return (
    <DropdownMenu.Portal>
      ...
      >
        ...
        {PERSONALIZE_GROUP_FIELDS.map((g) => (
          <DropdownMenu.Item
            key={g.id}
...
function FilterChip({
  ...
  filter: PersonalizeFilter;
  fieldLabel: string;
  options: string[];
  openOnMount: boolean;
  onChange: (patch: Partial<PersonalizeFilter>) => void;
  onRemove: () => void;
}) {
