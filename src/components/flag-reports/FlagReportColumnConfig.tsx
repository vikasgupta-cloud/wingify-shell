...
import {
  getFlagReportTableStore,
  type FlagReportRowDensity,
} from "@/store/flagReportTable";
import { cn } from "@/lib/utils";

const CHECKBOX_CLASS =
  "h-3.5 w-3.5 [&_svg]:size-3 disabled:cursor-not-allowed";

const DENSITIES: { key: FlagReportRowDensity; label: string; title: string }[] =
  [
...
export default function FlagReportColumnConfig({
  kind,
}: {
  kind: FlagReportKind;
}) {
  const columns = FLAG_REPORT_CONFIG[kind].columns;
  const { visibleColumns } = useActiveFlagReportViewState(kind);
  const updateDraft = getFlagReportViewsStore(kind)(
    (s) => s.updateActiveViewDraft
  );
  const resetColumns = getFlagReportViewsStore(kind)(
    (s) => s.resetActiveViewColumns
  );
  const tableStore = getFlagReportTableStore(kind);
  const rowDensity = tableStore((s) => s.rowDensity);
  const setRowDensity = tableStore((s) => s.setRowDensity);
  const [dragIndex, setDragIndex] = useState<number | null>(null);
  const [dropIndex, setDropIndex] = useState<number | null>(null);

  const byId = new Map(columns.map((c) => [c.id, c]));
  const available = columns.filter((c) => !visibleColumns.includes(c.id));
  const isLocked = (id: FlagReportColumnId) =>
    columns.find((c) => c.id === id)?.locked === true;

  const toggleColumn = (id: FlagReportColumnId) => {
