// Column show/hide + reorder — same UX as WE / Feature Flags ColumnConfig.
...
import {
  ...
  type RecommendationRowDensity,
} from "@/store/recommendationTable";
...
const isLocked = (id: RecommendationColumnId) =>
  RECOMMENDATION_COLUMNS.find((c) => c.id === id)?.locked === true;

export default function RecommendationColumnConfig() {
  ...
  const resetColumns = useRecommendationTableStore((s) => s.resetColumns);
  const rowDensity = useRecommendationTableStore((s) => s.rowDensity);
  const setRowDensity = useRecommendationTableStore((s) => s.setRowDensity);
  const [dragIndex, setDragIndex] = useState<number | null>(null);
  const [dropIndex, setDropIndex] = useState<number | null>(null);

  const byId = new Map(RECOMMENDATION_COLUMNS.map((c) => [c.id, c]));
  const available = RECOMMENDATION_COLUMNS.filter(
