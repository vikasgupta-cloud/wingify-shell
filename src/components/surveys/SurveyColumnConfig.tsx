// Column show/hide + reorder for Surveys — same UX as WE ColumnConfig.
...
import {
  useSurveyTableStore,
  type SurveyRowDensity,
} from "../../store/surveyTable";
import { cn } from "../../lib/utils";

const CHECKBOX_CLASS =
  "h-3.5 w-3.5 [&_svg]:size-3 disabled:cursor-not-allowed";

const DENSITIES: { key: SurveyRowDensity; label: string; title: string }[] = [
  { key: "compact", label: "S", title: "Compact" },
  { key: "default", label: "M", title: "Default" },
  { key: "comfortable", label: "L", title: "Comfortable" },
];

const isLocked = (id: SurveyColumnId) =>
  SURVEY_COLUMNS.find((c) => c.id === id)?.locked === true;

export default function SurveyColumnConfig() {
  const { visibleColumns } = useActiveSurveyViewState();
  const updateDraft = useSurveyViewsStore((s) => s.updateActiveViewDraft);
  const resetColumns = useSurveyViewsStore((s) => s.resetActiveViewColumns);
  const rowDensity = useSurveyTableStore((s) => s.rowDensity);
  const setRowDensity = useSurveyTableStore((s) => s.setRowDensity);
  const [dragIndex, setDragIndex] = useState<number | null>(null);
  const [dropIndex, setDropIndex] = useState<number | null>(null);

  const byId = new Map(SURVEY_COLUMNS.map((c) => [c.id, c]));
  const available = SURVEY_COLUMNS.filter((c) => !visibleColumns.includes(c.id));

  const toggleColumn = (id: SurveyColumnId) => {
