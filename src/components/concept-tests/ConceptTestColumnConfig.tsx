// Column show/hide + reorder for ConceptTests — same UX as WE ColumnConfig.
...
import {
  ...
  type ConceptTestRowDensity,
} from "../../store/conceptTestTable";
import { cn } from "../../lib/utils";

const CHECKBOX_CLASS =
  "h-3.5 w-3.5 [&_svg]:size-3 disabled:cursor-not-allowed";

const DENSITIES: { key: ConceptTestRowDensity; label: string; title: string }[] = [
  { key: "compact", label: "S", title: "Compact" },
  { key: "default", label: "M", title: "Default" },
  { key: "comfortable", label: "L", title: "Comfortable" },
];

const isLocked = (id: ConceptTestColumnId) =>
  CONCEPT_TEST_COLUMNS.find((c) => c.id === id)?.locked === true;

export default function ConceptTestColumnConfig() {
  const { visibleColumns } = useActiveConceptTestViewState();
  const updateDraft = useConceptTestViewsStore((s) => s.updateActiveViewDraft);
  const resetColumns = useConceptTestViewsStore((s) => s.resetActiveViewColumns);
  const rowDensity = useConceptTestTableStore((s) => s.rowDensity);
  const setRowDensity = useConceptTestTableStore((s) => s.setRowDensity);
  const [dragIndex, setDragIndex] = useState<number | null>(null);
  const [dropIndex, setDropIndex] = useState<number | null>(null);

  const byId = new Map(CONCEPT_TEST_COLUMNS.map((c) => [c.id, c]));
  const available = CONCEPT_TEST_COLUMNS.filter((c) => !visibleColumns.includes(c.id));

  const toggleColumn = (id: ConceptTestColumnId) => {
