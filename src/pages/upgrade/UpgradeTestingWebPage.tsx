/** Upgrade → Wingify Testing - Web — sticky section tabs + compact plan header.
 * Dummy data in upgradeTestingWeb.ts. Reuses shadcn Accordion, Checkbox, Button, Input, Tabs.
 */
...
export default function UpgradeTestingWebPage() {
  const [mtuIndex, setMtuIndex] = useState(DEFAULT_MTU_INDEX);
  ...
  const tier = MTU_TIERS[mtuIndex] ?? MTU_TIERS[DEFAULT_MTU_INDEX];
  const savePct =
  ...
  const filteredCategories = useMemo(() => {
    ...
    return FEATURE_CATEGORIES.map((cat) => {
      ...
      if (diffOnly) {
        rows = rows.filter((r) => {
          const vals = PLAN_COLUMNS.map((p) => r.cells[p.id]);
  ...
  const expandAllFaqs = () => {
    setFaqOpen(UPGRADE_FAQS.map((f) => f.id));
  };
      ...
      <section className="space-y-4 py-8">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Select the number of unique visitors that you plan to test in a
            month. This will be set as your plan&apos;s visitor quota.
          </p>
        </div>
        <div className="rounded-xl border border-border bg-background px-5 py-6 shadow-sm">
          <Slider
            min={0}
            max={MTU_TIERS.length - 1}
            step={1}
            value={[mtuIndex]}
            onValueChange={(v) => setMtuIndex(v[0] ?? DEFAULT_MTU_INDEX)}
      ...
      >
        ...
        >
          ...
          {PLAN_COLUMNS.map((plan) => {
            const isActiveCol = plan.id === ACTIVE_SUBSCRIPTION_PLAN;
